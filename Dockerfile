...
RUN apt-get update && apt-get install -y tini adb python3 python3-pip python3-venv && rm -rf /var/lib/apt/lists/*
...
# Fix streamableHttp bug in supergateway if present
RUN find /usr -name "webStandardStreamableHttp.js" -exec sed -i 's/throw new Error(`No connection established/console.error(`No connection established/g' {} + || true
